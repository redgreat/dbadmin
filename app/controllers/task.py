...
class TaskController:
    ...
    async def get_tasks(self,
                        ...
                        status: Optional[bool] = None) -> TaskList:
        """获取任务列表"""
        # 构建查询条件
        ...
        # 转换为响应模型
        return TaskList(
            items=[TaskInDB.model_validate(item) for item in items],
            total=total
        )
    ...
    async def get_task_logs(self,
                           ...
                           page: int = 1,
                           limit: int = 10,
                           status: Optional[str] = None) -> TaskLogList:
        """获取任务执行日志"""
        # 构建查询条件
        query = TaskLog.all()
        ...
        # 转换为响应模型
        return TaskLogList(
            items=[TaskLogInDB.model_validate(item) for item in items],
            total=total
        )
