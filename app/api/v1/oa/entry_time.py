...
@router.post("/validate", summary="验证入职时间")
async def validate_entry_time(req: Request, body: PositiveTimeRequest):
    try:
        codes = _clean_codes(body.codes)
        if not codes:
            return Fail(code=400, msg="人员工号不能为空")

        result = await oa_positive_time_service.validate_entry_time(codes)
        return Success(msg="验证完成", data=result)
    except Exception as e:
        logger.error(f"验证入职时间失败: {e}", exc_info=True)
        return Fail(code=500, msg=f"验证失败: {str(e)}")


@router.post("/execute", summary="修改入职时间")
async def execute_entry_time(req: Request, body: PositiveTimeRequest):
    try:
        codes = _clean_codes(body.codes)
        if not codes:
            return Fail(code=400, msg="人员工号不能为空")
        ...
        result = await oa_positive_time_service.update_entry_time(codes, body.positive_time)
        ...
        try:
            ...
            await create_operation_audit_log(
                ...
                username=username,
                module="OA运维",
                summary=f"入职时间修改: codes={','.join(codes)}, time={result.get('entry_time')}",
                method="POST",
                path="/api/v1/oa/entry-time/execute",
                status=200,
        ...
        logger.error(f"修改入职时间失败: {e}", exc_info=True)
