<template>
  <CommonPage show-footer>
    <n-form ref="formRef" :model="form" :rules="rules" label-placement="left" :label-width="100">
      ...
      <n-form-item label="修改时间" path="positiveTime">
        <n-date-picker v-model:value="form.positiveTime" type="datetime" clearable placeholder="请选择转正时间" />
      </n-form-item>
      <n-space>
...
defineOptions({ name: '转正时间修改' })

const message = useMessage()
const formRef = ref(null)
const form = ref({ codesText: '', positiveTime: null })
const results = ref([])
...
const rules = {
  ...
  positiveTime: [{ required: true, message: '请选择修改时间' }],
}
...
const columns = [
  ...
  { title: '人员Id', key: 'user_id', minWidth: 260 },
  { title: '当前转正时间', key: 'positive_time', minWidth: 180 },
]

const buildPayload = (includeTime = false) => {
  const payload = { codes: codes.value }
  if (includeTime) {
    payload.positive_time = dayjs(form.value.positiveTime).tz('Asia/Shanghai').format('YYYY-MM-DDTHH:mm:ss')
  }
  return payload
}
...
const handleValidate = async () => {
  ...
  try {
    const res = await api.validateOaPositiveTime(buildPayload())
    if (res.code === 200) {
...
const handleExecute = async () => {
  ...
  try {
    const res = await api.executeOaPositiveTime(buildPayload(true))
    if (res.code === 200) {
      const data = res.data || {}
...
const handleReset = () => {
  form.value = { codesText: '', positiveTime: null }
  results.value = []
}
</script>
